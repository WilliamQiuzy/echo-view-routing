...
| `asformer_official/v1` | official ASFormer epoch-120 model on the EV9V banks (+ per-bank eval, bank recipe, patch) | GTEA released models reproduce the paper's table; EV9V test pair bank F1@50 98.48 |
...
`models_frozen/` holds exactly the five baselines and nothing else: `stfm_official`, `mstcn_official`, `asformer_official`, `echoprime_view_classifier`, `echoviewclip_stage1`. All five baselines are frozen.
