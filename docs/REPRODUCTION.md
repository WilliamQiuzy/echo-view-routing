...
| ASFormer (Yi et al. 2021) | github.com/ChinaYi/ASFormer (MIT) | `e1bbe4f` | done: environment verified on GTEA; EV9V run `20260913-183307` (authors' eval.py, test pair bank: frame acc 99.33 / edit 99.12 / F1@50 98.48; multi 91.22 / 88.20 / 86.16); ladder cine macro-F1 0.959, boundary F1@0.5s 0.980; frozen as `models_frozen/asformer_official/v1` | authors' constants (120 ep, lr 5e-4, 10 layers, 64 maps, bz 1, channel mask 0.3); 2-line patch `third_party/patches/asformer-py3.patch` (NumPy `np.float`, removed deprecated `verbose=` kwarg); the L72 window-mask bug the authors mention is left as released |
